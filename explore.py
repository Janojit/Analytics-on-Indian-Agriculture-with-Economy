...
def rice():
    ...
    fig = px.bar(temp, 'State Name', 'RICE PRODUCTION (1000 tons)', color='RICE PRODUCTION (1000 tons)')
    st.plotly_chart(fig)
...
def q3():
    ...
    fig = px.bar(df1, 'State Name', 'Total Area (1000 ha)', color='Total Area (1000 ha)',title='Pie Plot Between States and Total Agriculture Land Area')
    st.plotly_chart(fig)
...
def q5():
    st.write('Join Plot Between Total Agriculture Land Area and Total Crop Production')
...
def q9():
    st.write('Join Plot Between Rainfall and Total Crop Production')
